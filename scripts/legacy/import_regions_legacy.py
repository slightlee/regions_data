...
def main():
    ...
    if os.path.exists(processed_dir):
        json_files = [f for f in os.listdir(processed_dir) if f.startswith("regions_") and f.endswith(".json")]
        if json_files:
            # 按文件名排序，获取最新的
    ...
    if not json_path:
        print(f"❌ 错误: 找不到清洗后的 JSON 数据文件 in {processed_dir}！")
        print("请先运行 scripts/clean_regions_data.py 处理数据")
        sys.exit(1)
